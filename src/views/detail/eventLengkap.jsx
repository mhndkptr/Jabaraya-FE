    ...
    <section className="border-black w-full min-h-screen px-5 md:px-28 mt-10 -z-10">
      ...
      <aside className="bg-jabarayaColors-200 flex flex-col w-full h-auto mt-5 rounded-xl lg:grid lg:grid-cols-[1fr,1fr] md:p-10 p-4 ">
        <div className="flex flex-col gap-2 md:gap-3 order-2 lg:order-1">
          <h2 className="lg:text-4xl text-[1.2rem] font-bold md:font-semibold lg:w-[700px]">{event.name}</h2>
          <p className="mt-1 p-2 bg-white w-[100px] rounded-md">{new Date(event.start_date).toLocaleDateString()}</p>
          <span className="text-xs border font-semibold border-jabarayaColors-700 p-1 rounded-md text-jabarayaColors-700 w-28">{getCategoryName(event.category_id)}</span>
          <div className="flex gap-2">
            <span className="material-symbols-outlined">location_on</span>
            <a href={event.location} target="blank" className="md:text-xl font-medium">
              {event.location}
            </a>
          </div>
          <div className="flex gap-2">
        ...
        <div className="flex justify-center items-center order-1 lg:order-2 mb-2">
          <img src={`${event.thumbnail}`} alt="" className="border border-black rounded-lg lg:w-[350px] lg:h-[300px]" />
        </div>
      </aside>
      <main className="flex flex-col justify-center items-center mt-10">
        <div className="flex flex-col justify-center items-center">
          <img src={`${event.thumbnail}`} alt="" className="border border-black rounded-lg md:w-[350px] md:h-[300px]" />
        </div>
        <article className="mt-20 font-medium w-full px-3 ">
          <p className="mb-5 text-justify text-[12px] md:text-[1rem]" dangerouslySetInnerHTML={{ __html: event.content }}></p>
