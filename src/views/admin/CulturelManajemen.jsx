...
export default function CulturelManajemen() {
  ...
  const handleEditSubmit = (e) => {
    ...
    axiosClient
      .post(`/cultures/${EditBudaya.id}`, formData, {
        headers: {
  ...
  const filteredBudaya = cultures.filter((cultures) => (cultures.title?.toLowerCase() || "").includes(searchQuery.toLowerCase()) || (cultures.content?.toLowerCase() || "").includes(searchQuery.toLowerCase()));
      ...
      <div className="p-4 mt-10 sm:ml-64 dark:bg-gray-900 ">
        ...
        <div className={EditBudaya ? "" : "hidden"}>
          <form className="p-4 md:p-5" onSubmit={handleEditSubmit}>
            <div className="grid gap-4 mb-4 grid-cols-2">
              <div className="col-span-2">
                ...
                <input
                  ...
                  className="block w-full p-2.5 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
              ...
              <div className="col-span-2">
                ...
                <CKEditor
                  ...
                  config={{
                    ckfinder: {
                      uploadUrl: `${import.meta.env.VITE_API_BASE_URL}/api/cultures/upload-image`,
                    },
                    toolbar: ["heading", "|", "bold", "italic", "link", "bulletedList", "numberedList", "|", "imageInsert", "blockQuote", "insertTable", "mediaEmbed", "undo", "redo"],
                  }}
                />
              </div>
              {/* update kategori id */}
            ...
            <button
              type="edit"
              className="inline-flex items-center text-black bg-transparent border border-black hover:bg-jabarayaColors-700 hover:text-white hover:border-none focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:hover:bg-gray-600 dark:hover:text-white"
            >
              Save Changes
            </button>
          </form>
          ...
          <div className="relative p-4 w-full max-w-md max-h-full">
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Add new cultures</h3>
                <button
                  type="button"
              ...
              <form className="p-4 md:p-5" onSubmit={handleSubmit}>
                <div className="grid gap-4 mb-4 grid-cols-2">
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      className="block w-full p-2.5 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-600 dark:border-gray-500 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 shadow-md hover:shadow-lg focus:outline-none focus:bg-white dark:focus:bg-gray-700 transition duration-200 ease-in-out"
                      required
                    />
                  </div>
                  <div className="col-span-2">
                ...
                <button
                  type="submit"
                  className="inline-flex items-center text-black bg-transparent border border-black hover:bg-jabarayaColors-700 hover:text-white hover:border-none focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:hover:bg-gray-600 dark:hover:text-white dark:text-white dark:border-none dark:bg-gray-700"
                >
        ...
        <div className="overflow-x-auto mt-5">
          ...
          <div className="flex items-center justify-end mb-2">
            <label htmlFor="sort" className="text-sm font-medium text-gray-900 me-2 dark:text-white">
              Sort By
            </label>
            <select id="sort" name="sort" value={sortOrder} onChange={handleSortChange} className="text-sm text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg p-2">
            ...
            <Table.Body className="divide-y">
              {currentBudaya.map((cultures) => (
                <Table.Row key={cultures.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">{cultures.title}</Table.Cell>
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">
                    <img src={`${cultures.thumbnail}`} alt={cultures.title} className="w-20 h-20 object-cover rounded-lg" />
                  </Table.Cell>
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">{cultures.content}</Table.Cell>
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">{cultures.category_id}</Table.Cell>
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">{new Date(cultures.created_at).toLocaleDateString()}</Table.Cell>
                  <Table.Cell>
                    <button onClick={() => handleEdit(cultures)} className="font-medium text-blue-600 hover:underline dark:text-blue-500">
                      Edit
                    </button>
                    <span className="mx-2">|</span>
                    <button onClick={() => handleDelete(cultures.id)} className="font-medium text-red-600 hover:underline dark:text-red-500">
                      Delete
                    </button>
                  </Table.Cell>
                </Table.Row>
          ...
          <div className="flex items-center justify-between mt-5">
            ...
            <div className="flex items-center">
              <button onClick={handlePreviousPage} disabled={currentPage === 1} className="text-sm text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg p-2.5">
                Prev
              </button>
              <button
                onClick={handleNextPage}
                disabled={currentPage === Math.ceil(sortedBudaya.length / BudayaPerPage)}
                className="text-sm text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg p-2.5 ms-2"
              >
                Next
              </button>
            </div>
